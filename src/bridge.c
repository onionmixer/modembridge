...
/* ========== Serial/Modem Thread Function (moved to level1_thread.c) ========== */
/*
 * serial_modem_thread_func() has been moved to level1_thread.c
 * This function handles Level 1 thread operations including:
 * - Serial port health checks
 * - Modem initialization verification
 * - Timestamp transmission
 * - Echo functionality
 * - Serial/Telnet data transfer
 *
 * The function is now available through #include "level1_thread.h"
 * when ENABLE_LEVEL1 is defined.
 */

#ifndef ENABLE_LEVEL1
/* Stub for when Level 1 is not enabled */
void *serial_modem_thread_func(void *arg)
{
    (void)arg; /* Unused parameter */
    MB_LOG_WARNING("serial_modem_thread_func called but Level 1 is not enabled");
    return NULL;
}
#endif
